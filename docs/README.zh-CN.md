...
升级到 `0.1.18` 时，应先保持四个开关为默认值，同时重启 MCP Server 和 Maintenance Daemon，避免不同版本的写入进程混用事务约定。公开 MCP 工具和参数没有删除；已有 SQLite 记忆继续作为权威数据，LanceDB 可由持久化校验任务修复。LanceDB 最低版本已提高到 `0.34.0`，固定在更早版本的环境必须先升级依赖。重启后执行：
