...
| # | 阶段 | 类型 | 算法 | 参数 |
|---|------|------|------|------|
| 0 | 自适应检索门 | gate | 短问候/命令跳过，强制词绕过 | CJK≥6, EN≥15 chars |
...
| 6 | Rerank（可选） | rerank | Ollama cross-encoder, 5s 超时 | 默认关闭, `PP_RECALL_RERANK=1` |
...
- Python 端：`memory_store` / `memory_update` / `memory_forget` / `memory_correct` 递增全局 `MEMORY_VERSION`（存储为模块级变量或 SQLite 单格表）
- Rust 端：`supply()` 入口检查传入的 `version` 参数，不匹配 → 从 SQLite 重载全部记忆 → 重建 BM25 DF 表
...
```
- Python supply() vs Rust supply() 对同一查询返回结果排序一致（相关性误差 <0.05）
