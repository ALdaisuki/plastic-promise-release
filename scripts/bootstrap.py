...
def bootstrap():
    ...
    print("  引擎版本: Python 回退版 (Rust 不可用时自动切换)")
    ...
    seed_memories = [
        # preference (偏好)
        ("用户偏好使用 Rust 编写后端服务，认为其性能和类型安全优于 Python", "experience", "user"),
        ("用户不喜欢未经过测试的代码直接合并到主分支", "experience", "user"),
        ("用户倾向于使用简洁的函数式编程风格，避免深层嵌套", "experience", "user"),
        # fact (事实)
        (
        ...
        ("当前 Rust 引擎使用 SQLite 做主存储，LanceDB 待环境就绪后接入", "experience", "system"),
        ("Ollama mxbai-embed-large 提供 1024 维嵌入向量，本地运行零延迟", "experience", "system"),
        # decision (决策)
        ("决定采用约定工程而非约束工程作为 AI 行为治理范式", "experience", "system"),
        ("决定优先实现核心方法的完整逻辑，辅助方法保持骨架", "experience", "system"),
        ("决定使用 PyO3 桥接 Rust 引擎，上层 Python 调用无感切换", "experience", "system"),
        # entity (实体)
        ("plastic_promise/core/context_engine.py 是上下文供应引擎的 Python 包装", "code", "system"),
        (
            "rust/context-engine-core 包含 HybridRetriever、SQLite、Weibull 衰减等核心组件",
            "code",
            "system",
        ),
        ("plastic_promise/mcp/server.py 暴露 25 个 MCP 工具，7 个工具组", "code", "system"),
        # event (事件)
        ("2026-06-28: 完成了从 P0 到 P2 的全部优先级任务实现", "task", "system"),
        ("2026-06-28: Ollama mxbai-embed-large 接入成功，dim=1024", "task", "system"),
        ("2026-06-28: 11 条核心原则成功注入 EntityGraph", "task", "system"),
        # pattern (模式)
        ("每次重大架构决策都会触发原则评估，形成决策→原则→反馈的闭环", "experience", "system"),
        ("记忆检索总是先尝试向量搜索，失败后回退文本匹配", "experience", "system"),
        (
    ...
    for content, mem_type, source in seed_memories:
        mid = engine.create_ordinary_if_absent(
            {
                "content": content,
    ...
    for mid, (content, _, _) in zip(stored_ids, seed_memories, strict=True):
    ...
    print("Bootstrap 完成")
    print(
        f"  原则节点: {sum(1 for n in engine._graph_nodes.values() if n.get('entity_type') == 'principle')}"
    )
    print(
        f"  Task Type 节点: {sum(1 for n in engine._graph_nodes.values() if n.get('entity_type') == 'task_type')}"
    )
