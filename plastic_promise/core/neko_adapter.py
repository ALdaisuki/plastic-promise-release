...
from plastic_promise.core.bus_client import InteropClient
...
TELEMETRY_DIR = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "..", ".interop", "telemetry"
)
...
class WSSlot:
    ...
    __slots__ = ("name", "url", "lanlan_name", "ws", "reader", "maintainer", "dead_event")
    ...
    async def maintain(self, backoff_min: float = 0.25, backoff_max: float = 1.5) -> None:
        ...
        while True:
            ...
            try:
                ...
                logger.debug(
                    "[WSSlot] %s connect failed: %s (backoff %.2fs)", self.name, e, backoff
                )
...
class NekoAdapter:
    ...
    def _on_zmq_event(self, event: dict[str, Any]) -> None:
        ...
        if self._loop is not None and not self._loop.is_closed():
            asyncio.run_coroutine_threadsafe(self._handle_zmq_event(event), self._loop)
        else:
            # 无事件循环时同步调用（用于测试）
    ...
    def _translate_zmq_event(self, event: dict[str, Any]) -> dict[str, Any] | None:
...
Examples:
  ...
  python bridge/neko_adapter.py --dry-run
    ...
    if args.dry_run:
        print("[NekoAdapter] Dry run — configuration:")
