...
class PluginLoader:
    ...
    def _activate_one(self, pack: PackInfo) -> bool:
        ...
        if not self._check_core_version(pack):
            logger.warning(
                ...
                pack.name, pack.min_core_version, self._core_version,
            )
            return False
        ...
        logger.info(
            ...
            pack.name, pack.pack_type, pack.version,
        )
        return True
    ...
    def _deactivate_one(self, pack_name: str) -> None:
        ...
        for slot in list(self._hooks.keys()):
            self._hooks[slot] = [
                h for h in self._hooks[slot] if h["plugin"] != pack_name
            ]
    ...
    def trigger_hooks(self, slot: str, context: dict) -> list[dict]:
        ...
        for hook in self.get_hooks(slot):
            try:
                ...
                logger.warning(
                    "Hook %s/%s failed: %s", slot, hook["plugin"], e
                )
    ...
    def _exec_mcp(self, hook: dict, context: dict) -> dict:
        ...
        mcp = McpSubprocessPlugin(
            command_str.split(), timeout=hook.get("timeout", 30)
        )
        if not mcp.start():
    ...
    def _exec_python(self, hook: dict, context: dict) -> dict:
        ...
        try:
            ...
            logger.warning(
                "Python hook %s import failed: %s", hook["plugin"], e
            )
        except Exception as e:
            logger.warning(
                "Python hook %s execution failed: %s", hook["plugin"], e
            )
        return {}
