...
async def scan_trust(engine) -> dict:
    ...
    try:
        ...
        # Get all targets
        targets = conn.execute("SELECT DISTINCT target FROM trust_scores").fetchall()
        ...
        for row in targets:
            ...
            if changes and changes["min_val"] is not None:
                ...
                history_count = conn.execute(
                    "SELECT COUNT(*) FROM trust_history WHERE target=? AND timestamp >= ?",
                    (target, fourteen_days_ago),
                ).fetchone()[0]
                ...
                if history_count >= 2 and movement < 0.01:
                    ...
                    current = conn.execute(
                        "SELECT trust, last_updated FROM trust_scores WHERE target=?", (target,)
                    ).fetchone()
                        ...
                        findings.append(
                            {
                                ...
                                "task_type_field": "investigate_trust",
                                "title": (
                                    f"信任停滞: {target} 14天波动<0.01 (当前{current['trust']:.2f})"
                                ),
                            }
                        )
            else:
        ...
        all_scores = conn.execute("SELECT target, trust FROM trust_scores").fetchall()
        ...
        for row in all_scores:
            ...
            for rank, threshold in sorted(
                RANK_THRESHOLDS.items(), key=lambda x: x[1], reverse=True
            ):
                ...
                if 0 < margin <= 0.03:
                    ...
                    if (
                        recent_deltas
                        and recent_deltas["total_delta"] is not None
                        and recent_deltas["total_delta"] < 0
                    ):
                        findings.append(
                            {
