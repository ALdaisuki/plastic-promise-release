...
def test_full_hunter_guild_lifecycle(e2e_db_path, monkeypatch):
    ...
    r = asyncio.run(handle_task_claim(engine, {
        "agent_name": "pi_fixer",
        "task_id": task_id,
        "trust_score": 0.60,
    }))
    ...
    r = asyncio.run(handle_task_heartbeat(engine, {
        "task_id": task_id,
        "agent_name": "pi_fixer",
    }))
    data = json.loads(r[0].text)
    ...
    r = asyncio.run(handle_task_complete(engine, {
        "task_id": task_id,
        "agent_name": "pi_fixer",
    ...
    r = asyncio.run(handle_task_verify(engine, {
        "task_id": task_id,
        "verdict": "accepted",
        "verified_by": "claude",
        "comment": "清理正确，LGTM",
